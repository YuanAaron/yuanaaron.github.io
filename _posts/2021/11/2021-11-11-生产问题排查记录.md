...
[运行时常量池、字符串常量池存放位置](https://blog.csdn.net/q5706503/article/details/84640762)：

> Jdk1.7之前运行时常量池逻辑包含字符串常量池，存在方法区，此时hotspot虚拟机堆方法区的实现为永久代；
>
> Jdk1.7字符串常量池被从方法区移到了堆中，这里没有提到运行时常量池，也就是说字符串常量池被单独拿到堆中，运行时常量池及其他还在方法区，也就是hotspot中的永久代；
>
> Jdk1.8的hotspot用元空间代替了永久代，这时字符串常量池还在堆，运行时常量池还在方法区，只不过方法区的实现从永久代变成了元空间。

永久代、元空间

> 元空间本质上和永久代类似，都是对JVM规范中方法区的实现。但元空间与永久代之间的【最大区别】是元空间并不在虚拟机中，而是使用本地内存。
...
```shell
# 生产报错：元空间大小设置为128m，系统启动时没有问题，运行了一段时候后元空间溢出。
# 1. 先把元空间的大小改大一些
-XX:MetaspaceSize=10m -XX:MaxMetaspaceSize=10m
# 2. 检查参数是否设置错误 
-XX:SoftRefLRUPolicyMSPerMB=1000 #默认值为1000，可能将该参数改小了
# 3. 检查是不是我们的代码或第三方框架在运行时频繁地创建了太多代理类
```
...
public class MetaspaceOom {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            enhancer.setUseCache(false); // 默认为true
            ...
            BusinessService businessService = (BusinessService) enhancer.create();
          	//System.out.println(businessService.getClass()); //1
...
VM启动参数设置为：-XX:MetaspaceSize=10m -XX:MaxMetaspaceSize=10m

现象：代码运行一小会儿就会报“元空间内存溢出”异常，尝试将元空间内存调大，比如50m，发现代码正常运行的时间变长，但最终还是会报该异常，这说明自己的代码或引用的第三方组件有“元空间溢出”的风险。

当前这个问题的解决办法是将【enhancer.setUseCache(false)】中的false改为true，原因是：

+ 当【enhancer.setUseCache(false)】时，产生的代理类都是不同的，不断产生新的代理类撑爆了元空间；
+ 当【enhancer.setUseCache(true)】时，生成的代理类会复用原先产生在缓存中的类，自始至终都只有一个代理类，所以不会发生元空间溢出。

这一点可以通过代码中1处输出的日志验证。

> 注意：测试发现，基于JDK的动态代理不会引发内存溢出。

## 直接内存溢出

现象：

> 报错：java.lang.OutOfMemoryError: Direct buffer error
>
> netty中的报错：io.netty.util.internal OutOfDirectMemoryError
